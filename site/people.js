(async function () {
  ...
  const renderFields = (person, omitCareer = false) => Object.entries(person.fields || {}).filter(([key]) => !omitCareer || !/^\d{4}_/.test(key)).map(([key, value]) => {
    const email = key.includes("mail") ? String(value).replace(/\s/g, "") : "";
    return `<div><dt>${escapeHtml(fieldLabels[key] || key.replaceAll("_", " "))}</dt><dd>${email ? `<a href="mailto:${escapeHtml(email)}">${escapeHtml(value)}</a>` : escapeHtml(value)}</dd></div>`;
  }).join("");
  const renderPerson = (person) => `<article class="person-card">${renderPhoto(person)}<div class="person-body"><h3>${escapeHtml(person.name)}</h3><p class="person-role">${escapeHtml(person.notes?.[0] || labels[person.group] || "")}</p><dl>${renderFields(person)}</dl></div></article>`;
  try {
    const data = await loadJson("people.json");
    const professor = data.groups.professor?.[0];
    const professorHtml = professor ? `<section class="people-section"><header><h2>${labels.professor}</h2><span>01</span></header><article class="professor-card">${renderPhoto(professor)}<div class="person-body"><p class="section-kicker">Principal investigator</p><h3>${escapeHtml(professor.name)}</h3><p class="person-role">${escapeHtml((professor.notes || []).slice(0, 3).join(" · "))}</p><dl>${renderFields(professor, true)}</dl><div class="career-list">${Object.entries(professor.fields || {}).filter(([key]) => /^\d{4}_/.test(key)).map(([key, value]) => `<div><time>${escapeHtml(key.replaceAll("_", " "))}</time><span>${escapeHtml(value)}</span></div>`).join("")}</div></div></article></section>` : "";
